const handler = ({cmd, data, response, logger, SETTINGS}) => {
    logger.debug('loraOtherConfigsHandler');
    logger.silly(data);

    switch (cmd) {
        case 'get_lora_other_conf': {
            logger.silly('handler case: get_lora_other_conf');
            const readConfig = require('./read-config.js').readConfig;

            const configs = readConfig({ SETTINGS, logger });
            const result = {
                cmd: 'get_lora_other_conf',
                result: true,
                msg: 'success',
                data: configs
            }
